...
{
    public async Task<DefaultResponse> GetAccountByUserGuidAsync(
        Guid userGuid,
        CancellationToken cancellationToken = default)
    {
        try
        {
            Context.Account.Entity.Account? account = await repository.GetAccountByUserGuidAsync(
                userGuid,
                cancellationToken)
                .ConfigureAwait(true);
    ...
    public async Task<DefaultResponse> UpdateAccountAsync(
        AccountModel accountModel,
        CancellationToken cancellationToken = default)
    {
        try
        {
            Context.Account.Entity.Account? account = await repository.GetAccountByGuidAsync(
                accountModel.Guid,
                cancellationToken)
                .ConfigureAwait(true);
            if (account is null)
            ...
            account = account with
            {
                ...
                RealName = accountModel.RealName,
            };
