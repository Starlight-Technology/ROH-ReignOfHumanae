...
public class PositionRepository : IPositionRepository
{
    ...
    {
        List<PlayerPositionGeo> all = await _collection
            .Find(_ => true)
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        BsonDocument[] pipeline = new[]
        {
            ...
            new BsonDocument
                ...
                { "maxDistance", 100000 },
                { "spherical", true },
                { "query", new BsonDocument("PlayerId", new BsonDocument("$ne", playerId)) }
                }),

            new BsonDocument(
            "$match",
            new BsonDocument("PositionY", new BsonDocument { { "$gte", position.Y - 50 }, { "$lte", position.Y + 50 } }))
        };

        List<PlayerPositionGeo> geoResults = await _collection
