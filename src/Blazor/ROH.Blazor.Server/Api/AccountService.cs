...
public class AccountService(ICustomAuthenticationStateProvider customAuthenticationStateProvider) : IAccountService
{
    ...
    public async Task<DefaultResponse?> CreateNewUser(UserModel user) => await _gateway.Post(
        Gateway.Services.CreateNewUser,
        user,
        await customAuthenticationStateProvider.GetToken());

    public async Task<DefaultResponse?> FindUserByEmail(string email) => await _gateway.Post(
        Gateway.Services.FindUserByEmail,
        email,
        await customAuthenticationStateProvider.GetToken());

    public async Task<DefaultResponse?> FindUserByUserName(string userName) => await _gateway.Post(
        Gateway.Services.FindUserByUserName,
        userName,
        await customAuthenticationStateProvider.GetToken());

    public async Task<DefaultResponse?> GetAccountByUserGuid(Guid userGuid) => await _gateway.Post(
        Gateway.Services.GetAccountByUserGuid,
        userGuid,
        await customAuthenticationStateProvider.GetToken());

    public async Task<DefaultResponse?> GetUserByGuid(Guid guid) => await _gateway.Post(
        Gateway.Services.GetUserByGuid,
        guid,
        await customAuthenticationStateProvider.GetToken());

    public async Task<DefaultResponse?> Login(LoginModel login) => await _gateway.Post(
        Gateway.Services.Login,
        login,
        await customAuthenticationStateProvider.GetToken());

    public async Task<DefaultResponse?> UpdateAccount(AccountModel account) => await _gateway.Post(
