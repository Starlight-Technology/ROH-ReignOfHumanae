...
public class VersionFileService(ICustomAuthenticationStateProvider customAuthenticationStateProvider) : IVersionFileService
{
    ...
    public async Task<DefaultResponse?> DownloadVersionFile(string FileGuid) => await _gateway.Get(
        Gateway.Services.DownloadFile,
        new { FileGuid },
        await customAuthenticationStateProvider.GetToken());

    public async Task<DefaultResponse?> GetAllVersionFiles(string VersionGuid) => await _gateway.Get(
        Gateway.Services.GetAllVersionFiles,
        new { VersionGuid },
        await customAuthenticationStateProvider.GetToken());

    public async Task<DefaultResponse?> UploadVersionFile(GameVersionFileModel Model) => await _gateway.Post(
